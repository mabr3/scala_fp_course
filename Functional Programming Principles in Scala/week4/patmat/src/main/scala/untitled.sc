...
trait Huffman extends HuffmanInterface:
   ...
   *   }
   */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(x => x).view.mapValues(_.size).toMap.toList
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
    case List() => List()
    case x :: xs => (Leaf(x._1, x._2) :: makeOrderedLeafList(xs)).sortWith(_.weight < _.weight)
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case x :: x2 :: xs => (makeCodeTree(x, x2) :: xs).sortWith(weight(_) < weight(_))
    case _ => trees
  }
   ...
   */
  def createCodeTree(chars: List[Char]): CodeTree =
    val x = (makeOrderedLeafList(times(chars)))
    println(x)
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    def traverse(t: CodeTree, b: List[Bit]) : List[Char] = t match {
      case Leaf(char, weight) =>
        ...
        else char :: traverse(tree, b)
      case Fork(left, right, chars, weight) =>
        if b.head == 0 then traverse(left, b.tail)
        else traverse(right, b.tail)
    }
    traverse(tree, bits)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    def helper(t: CodeTree)( ch: Char) : List[Bit] = t match {
      case Leaf(char, weight) => List()
      case Fork(left, right, _, weight) =>
        if (chars(left).contains(ch)) then 0 :: helper(left)(ch)
        else 1 :: helper(right)(ch)
    }
