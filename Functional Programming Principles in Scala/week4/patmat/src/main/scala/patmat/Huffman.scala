...
trait Huffman extends HuffmanInterface {
   ...
   *   }
   */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(identity).view.mapValues(_.size).toMap.toList
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case x :: xy :: xs => (makeCodeTree(x, xy)::xs).sortWith(weight(_) < weight(_))
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(t: CodeTree, b: List[Bit]) : List[Char] = t match {
      ...
      case Fork(left, right, chars, weight) => if (b.head==0) traverse(left, b.tail) else traverse(right, b.tail)
    }
    traverse(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def helper(t: CodeTree)(ch: Char) : List[Bit] = t match {
      case Leaf(_,_) => List()
      case Fork(left, right, _, _) => if (chars(left).contains(ch)) 0 :: helper(left)(ch) else 1 :: helper(right)(ch)
    }
