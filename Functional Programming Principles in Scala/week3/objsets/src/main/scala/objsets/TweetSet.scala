...
abstract class TweetSet extends TweetSetInterface:
  ...
  def helperRetweets(t: Tweet): Tweet
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = helperRetweets(Tweet("a","a",-1))

  def helperRetweets(most: Tweet) : Tweet =
      if (elem.retweets > most.retweets) right.helperRetweets(left.helperRetweets(elem))
      else right.helperRetweets(left.helperRetweets(most))
